...
from autonerfacto.autonerfacto_field import TCNNAutonerfactoField
from autonerfacto.autonerfacto_utils import (
    apply_sem_colormap,
    get_sem_metrics,
...
@dataclass
class AutonerfactoModelConfig(ModelConfig):
    """Autonerfacto Model Config"""

    _target: Type = field(default_factory=lambda: AutonerfactoModel)
    ...
    """Whether to predict semantics or not."""


class AutonerfactoModel(Model):
    """Autonerfacto model

    Args:
        config: Autonerfacto configuration to instantiate model
    """

    config: AutonerfactoModelConfig
    ...
    def populate_modules(self):
        ...
        # Fields
        self.field = TCNNAutonerfactoField(
            self.scene_box.aabb,
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        if self.config.use_semantics:
            ...
            outputs["semantics_colormap"] = apply_sem_colormap(semantic_labels)
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        (
            ...
            miou,
            miou_valid_class,
        ) = get_sem_metrics(outputs, batch)
        metrics_dict["sem_avg_acc"] = class_average_accuracy
        metrics_dict["sem_tot_acc"] = total_accuracy
        metrics_dict["sem_miou"] = miou
        metrics_dict["sem_miou_valid_class"] = miou_valid_class

        # Depth metrics
        if "depth" in batch.keys():
            depth_metrics = calculate_depth_metrics(
                batch["depth"].cpu().numpy(), outputs["depth"].cpu().numpy()
            )
            for k in depth_metrics.keys():
                metrics_dict[f"depth_{k}"] = depth_metrics[k]
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        loss_dict["semantics_loss"] = (
            self.cross_entropy_loss(outputs["semantics"], batch["semantics"]) * 0.02
        )
    ...
    def get_image_metrics_and_images(
        ...
        semantics_colormap = apply_sem_colormap(semantic_labels)
        semantics_target = apply_sem_colormap(batch["semantics"].long())

        combined_rgb = torch.cat([image, rgb], dim=1)
        combined_acc = torch.cat([acc], dim=1)
        combined_depth = torch.cat([depth], dim=1)
        combined_sem = torch.cat([semantics_target, semantics_colormap], dim=1)
