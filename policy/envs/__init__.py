...
class VecPyTorch:
    ...
    def reset(self):
        obs, info, autonerf_data, sem_gt, instance_sem_gt = self.venv.reset()
    ...
    def plan_act_and_preprocess(self, inputs):
        (
            ...
            sem_gt,
            instance_sem_gt,
        ) = self.venv.plan_act_and_preprocess(inputs)
        obs = torch.from_numpy(obs).float().to(self.device)
        reward = torch.from_numpy(reward).float()
        return obs, reward, done, info, autonerf_data, sem_gt, instance_sem_gt
