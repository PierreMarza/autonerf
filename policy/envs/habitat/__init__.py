...
from habitat import VectorEnv, make_dataset
...
from .autonerf_env import AutoNeRF_Env
...
def make_env_fn(args, config_env, rank):
    ...
    if args.agent == "sem_exp":
        ...
        env = AutoNeRF_Env(args=args, rank=rank, config_env=config_env, dataset=dataset)
...
def construct_envs(args):
    ...
    if args.split == "val" and args.num_processes == 1:
        scenes = [scenes[args.eval_scene_id]]
    ...
    for i in range(args.num_processes):
        ...
        if len(scenes) > 0:
            ...
            print(f"Thread {i}: {config_env.DATASET.CONTENT_SCENES}")
