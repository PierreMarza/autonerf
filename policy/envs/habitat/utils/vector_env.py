...
class VectorEnv:
    ...
    def reset(self):
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        obs, infos, autonerf_data, sem_gt, instance_sem_gt = zip(*results)

        self._is_waiting = False
        return (
            np.stack(obs),
            infos,
            np.stack(autonerf_data),
            np.stack(sem_gt),
            np.stack(instance_sem_gt),
        )
    ...
    def step_wait(self) -> List[Observations]:
        ...
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        self._is_waiting = False
        obs, rews, dones, infos, autonerf_data, sem_gt, instance_sem_gt = zip(*results)
        return (
            np.stack(obs),
            np.stack(rews),
            np.stack(dones),
