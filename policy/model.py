...
class Semantic_Policy(NNBase):
    def __init__(
        self, input_shape, recurrent=False, hidden_size=512, num_sem_categories=16
    ):
        super(Semantic_Policy, self).__init__(recurrent, hidden_size, hidden_size)
        ...
        self.linear1 = nn.Linear(out_size * 32 + 8 * 1, hidden_size)
...
class RL_Policy(nn.Module):
    def __init__(self, obs_shape, action_space, model_type=0, base_kwargs=None):
        ...
        if model_type == 1:
            self.network = Semantic_Policy(obs_shape, **base_kwargs)
        else:
...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.use_viewpoint_reward == 1:
            self.nb_additional_channels = args.nb_additional_channels
        else:
            self.nb_additional_channels = 0
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last):
        ...
        # Computing viewpoint binary values
        if self.nb_additional_channels > 0:
            bin_size = 360 / self.nb_additional_channels
            locs = current_poses.cpu().numpy()
            num_scenes = translated.shape[0]
            translated_viewpoints = []
            for e in range(num_scenes):
                r, c = locs[e, 1], locs[e, 0]
                loc_r, loc_c = [
                    int(r * 100.0 / self.resolution),
                    int(c * 100.0 / self.resolution),
                ]
                cells_locs = np.array(
                    np.where(translated[e, 0].detach().cpu().numpy() > 0.0)
                )
                viewpoints = np.zeros(
                    (
                        translated[e, 0].shape[0],
                        translated[e, 0].shape[1],
                        self.nb_additional_channels,
                    )
                )

                x_diffs = (loc_r - cells_locs[0]).astype(np.float64)
                y_diffs = (loc_c - cells_locs[1]).astype(np.float64)

                mask = 1 - (np.where((x_diffs == 0.0), 1, 0)) * (
                    np.where((y_diffs == 0.0), 1, 0)
                )
                ...
                viewpoints[cells_locs[0], cells_locs[1], direction_labels] = (
                    translated[e, 0, cells_locs[0], cells_locs[1]]
                    .detach()
                    .cpu()
                    .numpy()
                )
                translated_viewpoints.append(np.expand_dims(viewpoints, axis=0))

            translated_viewpoints = np.concatenate(translated_viewpoints, axis=0)
            translated_viewpoints = torch.Tensor(translated_viewpoints).to(self.device)
            translated_viewpoints = translated_viewpoints.permute((0, 3, 1, 2))
            translated = torch.cat([translated, translated_viewpoints], dim=1)

        maps2 = torch.cat((maps_last.unsqueeze(1), translated.unsqueeze(1)), 1)

        map_pred, _ = torch.max(maps2, 1)
        assert map_pred[0, 20:].sum().item() >= maps_last[0, 20:].sum().item()
