...
def raw2outputs(
    ...
    dists = torch.cat(
        [dists, torch.Tensor([1e10]).expand(dists[..., :1].shape).cuda()], -1
    )  # [N_rays, N_samples]
    ...
    weights = (
        ...
        * torch.cumprod(
            torch.cat(
                [torch.ones((alpha.shape[0], 1)).cuda(), 1.0 - alpha + 1e-10], -1
            ),
            -1,
        )[:, :-1]
    )
    ...
    if endpoint_feat:
        feat = raw[
            ..., -128:
        ]  # [N_rays, N_samples, feat_dim] take the last 128 dim from predictions
    ...
    if white_bkgd:
        ...
        if enable_semantic:
            sem_map = sem_map + (1.0 - acc_map[..., None])
